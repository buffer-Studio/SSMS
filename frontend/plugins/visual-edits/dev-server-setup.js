...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all google.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*google\.sh$/)) {
        return true;
      }

      // Allow all github.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*github\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="buffer@outlook.in" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="buffer@outlook.in" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
